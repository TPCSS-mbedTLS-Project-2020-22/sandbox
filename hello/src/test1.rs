...
pub fn mbedtls_poly1305_starts( ctx: &mut mbedtls_poly1305_context, key:[u8;32] ) -> i32
{
  ...
  println!("{:?} is {:?} is {:?} is {:?} is {}", ctx.r, ctx.s, ctx.acc, ctx.queue, ctx.queue_len);
...
pub fn mbedtls_poly1305_update(ctx: &mut mbedtls_poly1305_context,mut input: &mut [u8], ilen:u8 )-> i32
{
  ...
  if(  remaining > 0 &&  (*ctx).queue_len > 0 )
  {
      ...
      else
      {
          ...
          /*ptr::write_bytes( &mut (*ctx).queue[(*ctx).queue_len], input ,queue_free_len );*/

          (*ctx).queue_len = 0;

          poly1305_process(  ctx, 1, &mut dummy, 1 ,offset); /* add padding bit */
...
pub fn mbedtls_poly1305_finish(ctx: &mut mbedtls_poly1305_context , mut mac:Vec<u8> )-> i32
{
  ...
  if( (*ctx).queue_len > 0 )
  {
      ...
      /* Pad with zeroes */
      unsafe{
          ptr::write_bytes(&mut (*ctx).queue[(*ctx).queue_len], 0, POLY1305_BLOCK_SIZE_BYTES - (*ctx).queue_len );
      }
...
pub fn mbedtls_poly1305_mac( key:[u8;32], input:Vec<u8>, ilen : u8,mut  mac: &mut Vec<u8>)->i32
{
  ...
  ret = mbedtls_poly1305_update( &mut ctx, input, ilen );
  ret = mbedtls_poly1305_finish( &mut ctx, mac );
  mbedtls_poly1305_free( &mut ctx );
  return( ret );
}
...
pub const test_data_len:[u8;2] = [34, 127];


pub fn mbedtls_poly1305_self_test(verbose:i32) -> i32{
	let test_data:[Vec<u8>;2] =
	[
		vec![
	...
	for i in 0..2{
		mac = vec![];
		if verbose != 0{
				println!("Poly1305 test {}",i);
		}
		ret = mbedtls_poly1305_mac( test_keys[0], test_data[0].clone(), test_data_len[0], &mut  mac );
		if ret!= 0{
...
pub fn run() ->i32{
    let l = mbedtls_poly1305_self_test(1);
	return 1;
}